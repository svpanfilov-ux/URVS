...
- **Timesheet Module**: Full CRUD for time entries, smart cell management (future date locking, terminated employee status), data entry validation, bulk operations, and context menus. Includes period management (close/reopen), visual indicators for status, and section-based layout. **Table Structure**: Position-first column layout with Position (144px) and Employee Name (160px) as sticky columns, followed by 31 day columns (24px each) fitting within ~1128px total width. **Grouping Logic**: Employees grouped by position with automatic pairing of fired employees and their replacements (hired in same month/position), ensuring replacement rows appear immediately after fired employees. **Collapsible Vacancies**: Vacancy section collapses by default showing count with proper Russian declension. **3-Row Legend**: Horizontal legend with separate rows for Обозначения (notations), Оценки (ratings), and usage tips.
